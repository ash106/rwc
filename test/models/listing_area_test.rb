...
class ListingAreaTest < ActiveSupport::TestCase
  ...
  test 'must have a unique name' do
    copy_cat = ListingArea.new(name: listing_areas(:slv_central).name)
  ...
  test 'parse_kml creates polygon json attribute' do
    ...
    updated_listing_area = ListingArea.find(listing_area.id)
    assert_equal "Polygon", updated_listing_area.polygon["type"]
    assert_equal [-111.923637152839,40.6885149314539], updated_listing_area.polygon["coordinates"][0][0]
    assert_equal [-111.923637152839,40.6885149314539], updated_listing_area.polygon["coordinates"][0][-1]
  end
